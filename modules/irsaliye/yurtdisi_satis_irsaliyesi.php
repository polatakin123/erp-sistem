...
$query = "SELECT MAX(CAST(SUBSTRING(FISNO, 4) AS UNSIGNED)) as max_no FROM stk_fis WHERE TIP='13'";
$stmt = $db->query($query);
$result = $stmt->fetch(PDO::FETCH_ASSOC);
$next_no = 'YDS' . str_pad(($result['max_no'] ?? 0) + 1, 6, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $query = "INSERT INTO stk_fis (
                    ...
                    NOTLAR, DURUM, SUBEID, DOVIZID, DOVIZFIYAT, DOVEZTOPLAM
                ) VALUES (
                    1, '13', :fisno, :tarih, NOW(), :cari_id, :depo_id,
                    :toplam_tutar, 0, 0, 0, 0,
                    :toplam_tutar, 0, :toplam_tutar, :cari_adi, 0, 0,
                    :notlar, 1, 1, :doviz_id, :doviz_kuru, :doviz_toplam
                )";
        ...
        $stmt->execute([
            ...
            'cari_adi' => $cari_adi,
            'notlar' => $_POST['notlar'] ?? '',
            'doviz_id' => $_POST['doviz_id'] ?? 2, // 2: USD olarak varsayılan
            'doviz_kuru' => $_POST['doviz_kuru'] ?? 1,
            'doviz_toplam' => $_POST['doviz_toplam'] ?? $_POST['toplam_tutar']
        ]);
        ...
        $query = "INSERT INTO stk_fis_har (
                    ...
                    :sirano, 1, '13', :irsaliye_id, :tarih, 1,
...
// Sayfa başlığı
$pageTitle = "Yurt Dışı Satış İrsaliyesi";
            ...
            <form method="post" id="irsaliyeForm">
                ...
                <div class="row mb-3">
                    <div class="col-md-3">
                        ...
                        <select class="form-select" name="doviz_id">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" class="form-control" name="doviz_kuru" id="dovizKuru" step="0.0001" min="0.0001" value="1">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Notlar</label>
                        <textarea class="form-control" name="notlar" rows="1"></textarea>
                    </div>
                </div>
                ...
                <div class="table-responsive mb-3">
                    <table class="table table-bordered" id="kalemTable">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="3" class="text-end"><strong>Genel Toplam (Döviz):</strong></td>
                                <td colspan="1">
                                    <input type="number" class="form-control" name="doviz_toplam" id="dovizToplam" step="0.01" readonly>
                                </td>
                                <td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Döviz kuru değiştiğinde toplamı yeniden hesapla
    document.getElementById('dovizKuru').addEventListener('input', hesaplaGenelToplam);
    ...
    // Genel toplam hesaplama fonksiyonu (döviz dahil)
    function hesaplaGenelToplam() {
        ...
        // Döviz hesaplamaları
        const dovizKuru = parseFloat(document.getElementById('dovizKuru').value) || 1;
        const tlToplam = toplam * dovizKuru;
        
        document.getElementById('dovizToplam').value = toplam.toFixed(2);
        document.getElementById('genelToplam').value = tlToplam.toFixed(2);
    }
